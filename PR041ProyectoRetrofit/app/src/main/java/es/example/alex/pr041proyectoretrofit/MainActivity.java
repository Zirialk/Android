...
public class MainActivity extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener, AlumnoAdapter.OnItemClickListener {
    ...
    private Bdd.BddInterface servicio;
    ...
    private void listarAlumnos(){
        ...
        servicio.listarAlumnos().enqueue(new Callback<List<Alumno>>() {
            @Override
            public void onResponse(Response<List<Alumno>> response) {
                //Reemplaza los alumnos anteriores por los actuales, evitando que salgan repetidos.
                mAlumnoAdapter.replaceAlumnos(response.body());
                //Oculta el circulo de cargar, cuando ha terminado de colocar los alumnos en la lista.
                swlPanel.setRefreshing(false);
            }

            @Override
            public void onFailure(Throwable t) {
                makeToast("No se ha podido encontrar los alumnos");
                swlPanel.setRefreshing(false);
            }
        });
    }
