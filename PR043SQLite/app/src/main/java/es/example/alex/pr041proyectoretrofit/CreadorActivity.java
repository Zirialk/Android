...
public class CreadorActivity extends AppCompatActivity{
    ...
    private void confirmarCreacion(){
        ...
        if(DAO.getDAO(this).createAlumno(alumno) != -1)
    ...
    private void confirmarEdicion(){
        ...
        if(DAO.getDAO(this).updateAlumno(alumnoEditado))
    ...
    private Alumno crearAlumnoFromForm(){
        ...
        return new Alumno(txtNombre.getText().toString(), spCurso.getSelectedItem().toString(), txtTlf.getText().toString(), edad, txtDireccion.getText().toString(), rbSi.isChecked() , txtFoto.getText().toString());
